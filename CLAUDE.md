...
## 通常検索パイプライン統合プロジェクトにおける例外

現在進めている「通常検索まわりの表示・料金計算パイプライン統合」（PROJECT_HANDOFF.md既知の保留事項21・22参照）に限り、上記の「旧関数を無期限に残す」方針に、以下の例外を設けます。

- 新しい統合パイプライン（IC境界ベースの判定方式）を実装した後、実車確認で新方式が十分信頼できると確認できた段階で、置き換えられた旧方式（座標ベースの点対点サンプリング`roadSequence`/`displayRoadSequence`、テキストキーワード判定`classifyStepsByRoadType`の一部等）は、**明示的に削除してよい**こととします
- ただし、これは「即座に削除してよい」という意味ではなく、「新方式の検証が完了し、ユーザーが削除の指示を出した時点で、実行してよい」という意味です。検証前の段階で自主的に削除しないでください
- この例外は、今回のパイプライン統合プロジェクトに限った運用です。他の旧UI・旧関数の扱いについては、引き続き従来通り「安易に削除しない、削除する場合はユーザーの明示的な許可を得る」という基本方針に従ってください
