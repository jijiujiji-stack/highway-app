...
### 8. keiyo（京葉道路）エリアの個別残課題

- 京葉市川IC・穴川ICは複数地点にまたがる構造のため、将来的にレコード分割設計（`entrancePoint`/`exitPoint`等）を検討する（既知の保留事項3「浦安など入口・出口分離IC対応」と同種の課題）
- 四街道IC・佐倉IC・富里IC・大栄IC・佐原香取IC・酒々井ICの座標精密確認が未了
- 成田ICと新空港自動車道側との関係（接続構造）の確認が必要

---

### 9. joban（常磐道）エリアの残課題

三郷IC・日立中央IC・高萩IC・堤通（首都高）/加平（首都高）/八潮南（首都高）の座標・構造確認が残っています。
