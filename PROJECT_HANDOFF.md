...
**2026-08-03追記・対応完了**：IC_MASTERに新エリア`higashiMito`を新設し、水戸大洗IC・ひたちなかICを新規登録した（座標は複数ソースで確認、ひたちなかICのみソース間で約310mの不一致がありnoteに記載・信頼度「中」）。境界ICである水戸南IC（kitakanto側）に`connection: true`・`connectedRoads: ["kitakanto", "higashiMito"]`を追加した。

料金カテゴリは、ひたちなかIC⇔水戸南IC間の実際の運賃実績（12.9km・370円）と既存nexco式（約510円、約38%・140円の過大計上）の乖離を確認した上で、データ点が1つのみのため専用カテゴリ新設は見送り、既存の`nexco`カテゴリをそのまま採用する方針とした（コメントに乖離の実数値と判断根拠を記録済み）。

実車確認2ルートで、末尾フォールバック（黄色表示）の解消を確認した。

- ひたちなかIC発→伊香保温泉：「NEXCO入口：ひたちなかIC」が正常表示、`tollEntryCount: 1`、ETC概算約4,980円
- 大洗海岸→伊香保温泉（当初の不具合発覚ルート）：「NEXCO入口：水戸大洗IC」が正常表示、境界判定成功（しきい値までの余裕約358m）、`tollEntryCount: 1`、ETC概算約4,860円

ひたちなかICの座標信頼度（中、ソース間約310m不一致）は残課題。実車確認上は問題なく機能したが、追加ソースでの裏取りは今後の余地として残る。
