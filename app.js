...
function buildFullPassedIcSequenceFromTollSectionRange(
    ...
    // 区間境界（entranceIc/exitIc）が実際に未解決（null）の場合のみ、
    // 対応するicName（ヒント込みの場合がある）をpushUnresolvedMarkerへの
    // labelとして使う。ic自体は解決しているがroute DistanceCandidateIcs
    // 側でたまたま一致が取れなかった場合（理論上のみ）に、実在するIC名を
    // 誤って「IC不明」として表示しないためのガード。
    const getUnresolvedBoundaryLabel = (ic, icName) =>
        ic ? null : icName;
    ...
    // 【IC不明時のヒント表示・再利用】labelには、section.entranceIcName/
...
const SHUTO_IC_MASTER = [
    ...
    },
    {
        ...
        lat: 35.708561,
        lng: 139.776389,
        entranceLat: 35.708561,
        entranceLng: 139.776389,
        exitLat: 35.708561,
        exitLng: 139.776389,
        entranceSelectable: true,
        exitSelectable: true,
        ...
        note: "【2026-07-25再照合・座標修正】従来の座標（35.712,139.779）は未検証の値だった。keiyoエリア側の重複コピーが2026-07-18に二次情報源で35.708561,139.776389を確認済みだったが、確信度は中程度（一次情報未到達）との留保付きだった。今回、NAVITIME「首都高速1号上野線 上野出入口」で同一の35.708561,139.776389を確認し、独立した2ソース（keiyo側の二次情報源＋NAVITIME）が一致したため、確信度を高いと判断し本体側の座標を修正した。"
    },
    {
        id: "shuto-1-honcho",
        displayName: "本町",
    ...
    },
    {
        ...
        note: "既存IC_MASTERから正規化登録。現時点では未参照。【2026-07-25情報引き継ぎ】NEXCO側keiyoエリアの重複コピー削除に伴い、以下の留保情報を引き継ぐ：【構造変更確認・selectableは変更保留】首都高速道路株式会社プレスリリース（2018年）およびWikipedia「大井南出入口」記事で、2018年4月15日の湾岸線本線料金所（東行き）撤去に伴い、東行きの「大井南入口」は「中央環状線大井南入口」に名称変更され、湾岸線への流入はできなくなり中央環状線・羽田線専用になったことを確認した（湾岸線東行きを使う場合は約1.3km先の「大井入口」を利用）。西行きの出口・入口は存続。この構造変更が現在のgoogleName「首都高速湾岸線 大井南出入口」の登録内容と整合しているか要再確認だが、entranceSelectable/exitSelectableの変更は対象外のためtrue/trueのまま変更していない。座標もMapFan等一次情報に到達できず未確認のため、既存値(35.589,139.756)から変更していない。"
    },
    {
    ...
    kitakanto: {
        ...
        exits: [
            ...
            },
            {
                ...
                lng: 140.487599,
                connection: true,
                connectedRoads: ["kitakanto", "higashiMito"],
                ...
                note: "【2026-07-29調査・新規登録】北関東自動車道の終点（東側）、東水戸道路の起点を兼ねるIC。MapFan「水戸南ＩＣ（北関東自動車道（栃木茨城区間））【入口（上り）】」(36.3325435,140.4881631)、ユーザーが追加確認したNAVITIME「東行き出口/西行き入口」(36.328788,140.486462)、Wikipedia(36.329561,140.488208)、Yahoo!地図(36.3307543,140.487562)の4ソースが経度でほぼ一致（140.4865〜140.4882）、緯度に約370m幅で収まったため、4ソース平均値(36.330412,140.487599)を採用。Wikipediaによれば当ICを境に西側が高速自動車国道（北関東道終点）、東側が一般有料道路（東水戸道路起点）となる複合構造で、フルIC（入口2ブース、出口3ブース）。entranceSelectable/exitSelectableはtrue/true。entranceLat/Lng・exitLat/Lngともに上記平均値を暫定採用、lat/lngも同値。北関東道／東水戸道路境界という特殊構造のため、実車確認時に上り・下り・東水戸道路側それぞれの挙動を要確認。【2026-08-03追記】既知の保留事項30（東水戸道路未登録）への対応として、東水戸道路エリア（higashiMito、水戸大洗IC・ひたちなかIC）を新規登録した。本ICはkitakanto／higashiMitoの境界ICのため、既存の高崎JCT（kitakanto⇔kanetsu）・宇都宮IC（tohoku⇔nikkoUtsunomiya）と同じパターンに合わせ、connection:true・connectedRoads:[\"kitakanto\",\"higashiMito\"]を追加した（order・座標・note本文は変更なし）。"
            }
        ]
    },

    // higashiMito: 東水戸道路。NEXCO東日本管理の一般有料道路で、水戸南IC
    // （北関東自動車道と接続、既存kitakantoエリアに登録済み）〜ひたちなかIC
    // （常陸那珂有料道路と接続）間の約12kmを結ぶ。
    // 【2026-08-03新規追加】既知の保留事項30（2026-07-29発見）への対応。
    // 大洗海岸→伊香保温泉ルートの実車確認で、実際には東水戸道路（水戸大洗IC
    // 付近）を走行しているにもかかわらずIC_MASTER未登録のため候補IC 0件と
    // なり、末尾フォールバックにより「NEXCO入口：水戸南IC」が黄色（フォール
    // バック発動）表示になった問題への対応として、水戸大洗IC・ひたちなかIC
    // の2件を新規登録した。
    // 【料金カテゴリの判断・2026-08-03ユーザー確認済み】ひたちなかIC⇔
    // 水戸南IC間の実際の運賃実績は12.9km・370円（一般・ETCとも同額）。
    // 既存nexcoカテゴリの式（150円+24.6円/km*12.9km=467円、*税率1.1=
    // 514円→丸め510円）を適用すると510円となり、実際より約140円
    // （約38%）高く見積もる。日光宇都宮道路がnexco式で45〜85%高く見積
    // もっていた問題（専用カテゴリ新設の理由）と同種の過大計上であり、
    // 規模はやや小さいものの「近い水準」と言える差ではない。理由は以下
    // の通り：
    // (1) 運賃実績が12.9km・370円の1点のみで、日光宇都宮道路のような
    //     複数距離点からのbase+per-km式の当てはめができない
    // (2) 東水戸道路は水戸南IC・水戸大洗IC・ひたちなかICの3IC構成で部分
    //     区間の通行もあり得るため、aqualineのような単純な定額(fixed)化も
    //     適さない
    // (3) 全体ルート（例：大洗海岸→伊香保温泉）に対する140円の絶対額は
    //     小さいが、東水戸道路単体（大洗⇔ひたちなか等）が移動全体を占める
    //     利用では約38%の相対誤差がそのまま体感される
    // 以上を踏まえ、ユーザーと協議のうえ「既存nexcoカテゴリをそのまま使い、
    // 実車確認・追加の運賃データが集まった時点で再検討する」方針で合意した
    // （sourceAreaKey:"higashiMito"はTOLL_ROAD_CATEGORY_RULESのどのidとも
    // 一致しないため、resolveIcTollCategoryIdは自動的に汎用"nexco"を返す。
    // コード変更不要）。
    // sourceAreaKeyをkitakantoに統合せず新エリアとして分離したのは、
    // resolveTollSectionNexcoRoadLabel・resolveIcCategoryLabel等が
    // IC_MASTER[sourceAreaKey].labelをそのまま道路名表示に使うため、
    // kitakantoに統合すると東水戸道路区間が「北関東道方面」と誤表示される
    // ためである（日光宇都宮道路をtohokuに統合せず新エリアにしたのと同じ
    // 理由）。この分離に伴い、境界ICである水戸南IC（kitakanto側）に
    // connection:true・connectedRoads:["kitakanto","higashiMito"]を追加
    // 済み（高崎JCT・宇都宮ICと同じパターン）。
    higashiMito: {
        label: "東水戸道路",
        exits: [
            {
                order: 1,
                displayName: "水戸大洗IC",
                googleName: "東水戸道路 水戸大洗インターチェンジ",
                lat: 36.339796,
                lng: 140.544187,
                entranceSelectable: true, exitSelectable: true, entranceLat: 36.339796, entranceLng: 140.544187, exitLat: 36.339796, exitLng: 140.544187,
                note: "【2026-08-03調査・新規登録】既知の保留事項30（東水戸道路未登録）への対応。東水戸道路（水戸南IC〜ひたちなかIC）区間のIC。Wikipedia(北緯36度20分23.1秒 東経140度32分38.93秒=36.339750,140.544147)・Yahoo!地図(36.339849,140.544215)・高速.jp(36.33979,140.5442)の3独立ソースが半径15m程度で近接一致したため、この3ソース平均(36.339796,140.544187)を採用。MapFan「水戸大洗ＩＣ（東水戸道路）【入口】」(36.3402043,140.5397279)・【出口】(36.3401551,140.5398299)は入口・出口間で約10mと自己一致するが、上記3ソースクラスタからは経度で約390m西にズレる外れ値のため今回は不採用。フルIC（入口2ブース：ETC専用1・一般1、出口3ブース：ETC専用1・一般2、MapFan記載）。entranceSelectable/exitSelectableはtrue/true。国道51号と接続。entranceLat/Lng・exitLat/Lngともに上記3ソース平均値、lat/lngも同値。実車確認時、MapFan外れ値との位置関係の再検証を推奨。"
            },
            {
                order: 2,
                displayName: "ひたちなかIC",
                googleName: "東水戸道路 ひたちなかインターチェンジ",
                lat: 36.374580,
                lng: 140.575436,
                entranceSelectable: true, exitSelectable: true, entranceLat: 36.374580, entranceLng: 140.575436, exitLat: 36.374580, exitLng: 140.575436,
                note: "【2026-08-03調査・新規登録・要再確認】既知の保留事項30（東水戸道路未登録）への対応。東水戸道路の終点（東側）、常陸那珂有料道路の起点を兼ねる複合IC構造（本線料金所併設、Wikipedia記載）。Wikipedia(北緯36度22分28.27秒 東経140度34分31.52秒=36.374519,140.575422)とYahoo!地図(36.374640,140.575449)が半径15m程度で近接一致した一方、MapFan「ひたちなかＩＣ（東水戸道路）【出口（下り）】」(36.373079,140.578616)・「ひたちなかＩＣ（常陸那珂有料道路）【入口（下り）】」(36.373006,140.578622)は互いに約10mで一致するが、Wikipedia/Yahoo!地図クラスタから約310m東にズレており、2つの独立クラスタが解消しない状態（マピオンの地図中心座標36.373042,140.578619もMapFan側クラスタに近い値だが、施設名・座標をページ本文で直接確認できず参考止まり。NAVITIME個別ページは今回403エラーで直接確認できず未反映）。東水戸道路→常陸那珂有料道路の境界という複合構造のため、Wikipedia/Yahoo!地図側が施設の代表点、MapFan側が本線料金所寄りの実際のランプ座標を指している可能性がある。今回はWikipedia・Yahoo!地図の2独立ソース一致を優先し暫定採用(36.374580,140.575436)したが、確信度は中程度に留まる。フルIC（上下線とも入口・出口あり、Wikipedia記載）。entranceSelectable/exitSelectableはtrue/true。国道245号と接続。entranceLat/Lng・exitLat/Lngともに上記2ソース平均値、lat/lngも同値。実車確認、または追加ソースでの裏取りを強く推奨。"
            }
        ]
    },
...
function getAllRouteAnalysisIcDefinitions() {
    ...
    const definitions = dedupeIcDefinitionsByIdentity([
        ...getAllNexcoIcDefinitions(),
        ...getAllShutoIcDefinitions()
    ]);

    // 【DEBUG-UCHIMAWARI統合検証・一時的】「入谷」の本体（SHUTO_IC_MASTER）と
    // Uchimawariミラー（IC_MASTER内shuto1UenoUchimawari）が、
    // dedupeIcDefinitionsByIdentityで実際に1件へ統合されているかを確認するための
    // 一時ログ。既存の戻り値・判定ロジックには一切影響しない。調査完了後、
    // このブロックごと削除すること。
    const debugIriyaEntries = definitions.filter(
        ic => ic.displayName === "入谷"
    );

    if (debugIriyaEntries.length > 0) {
        console.log(
            "[DEBUG-UCHIMAWARI統合検証・一時的] 入谷エントリ数:",
            debugIriyaEntries.length,
            debugIriyaEntries.map(ic => ({
                source: ic.source,
                sourceAreaKey: ic.sourceAreaKey,
                googleName: ic.googleName,
                lat: ic.lat,
                lng: ic.lng,
